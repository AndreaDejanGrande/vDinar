...
BOOST_AUTO_TEST_SUITE(vcrypt_tests)

BOOST_AUTO_TEST_CASE(vcrypt_hashtest)
{
...
#if defined(USE_SSE2)
        // Test SSE2 vcrypt
        vcrypt_1_1_256_sp_sse2((const char*)&inputbytes[0], BEGIN(vcrypthash), scratchpad, n);
        BOOST_CHECK_EQUAL(vcrypthash.ToString().c_str(), expected[i]);
#endif
        // Test generic vcrypt
        vcrypt_1_1_256_sp_generic((const char*)&inputbytes[0], BEGIN(vcrypthash), scratchpad, n);
        BOOST_CHECK_EQUAL(vcrypthash.ToString().c_str(), expected[i]);
    }
}
