...
#if defined(NDEBUG)
# error "vDinar cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5ef756bea8ef0fe2bede9461d8c195c622f783b132b06ea3941c9e956639a6f9"); //16527bde694d8e45c8b53bb76b323751fe4d4753bd11c10537619e53b4959c25
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // vDinar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "vDinar Signed Message:\n";
...
{
    // vDinar: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (!ReadFromDisk(pindex->GetBlockPos(), vCrypt_N(pindex->nHeight)))
        return false;
...
int64 static GetBlockDonations(int nHeight, int64 nFees)
{
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // vDinar: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // vDinar: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // vDinar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
 ...
 bool fNegative;
 bool fOverflow;
 bnTarget.SetCompactDeclared(nBits, &fNegative, &fOverflow);
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
    ...
    if (blockUndo.vtxundo.size() + 2 != vtx.size())
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].GetValueOut() > GetBlockReward(pindex->nHeight, nFees))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), GetBlockReward(pindex->nHeight, nFees)));

    if (vtx[1].GetValueOut() != GetBlockDonations(pindex->nHeight, nFees))
        return state.DoS(100, error("ConnectBlock() : coinbase giving wrong donations (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[1].GetValueOut(), GetBlockDonations(pindex->nHeight, nFees)));
...

    {
        ...
        CScript scriptDonationsPubKey;
        scriptDonationsPubKey.SetDestination(donationsAddress.Get());

        if (vtx[1].vout[0].scriptPubKey != scriptDonationsPubKey)
            return state.DoS(100, error("ConnectBlock() : block submitter trying to change donations' address!"));
    }
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  vcrypt_n=%u  log2_work=%.8g  tx=%lu  date=%s progress=%f\n",
      hashBestChain.ToString().c_str(), nBestHeight, vCrypt_N(nBestHeight),
      log(nBestChainWork.getdouble())/log(2.0), (unsigned long)pindexNew->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexBest->GetBlockTime()).c_str(),
...
bool CBlock::CheckBlock(const CBlock* block, uint32_t nHeight, CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // First (and second) transaction(s) must be coinbase, the rest must not be
    if (vtx.empty() || !vtx[0].IsCoinBase())
        return state.DoS(100, error("CheckBlock() : first transaction is not coinbase"));
    // Second (and first) transaction(s) must be coinbase, the rest must not be
    if (!vtx[1].IsCoinBase()) {
        if(vtx[1].vin.size() != 1)
        {
        ...
        {
            return state.DoS(100, error("CheckBlock() : second transaction's prevout not null"));
        }
        return state.DoS(100, error("CheckBlock() : second transaction is not coinbase"));
    }
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinBase())
            return state.DoS(100, error("CheckBlock() : more than two coinbase"));
    ...
    /*if(GetTxHash(0) != GetTxHash(1)) {
        return state.DoS(100, error("CheckBlock() : wrong coinbase transactions"), true);
    }*/

    for (unsigned int i=1; i<vtx.size(); i++) {
        uniqueTx.insert(GetTxHash(i));
    }
    if (uniqueTx.size() != (vtx.size() - 1))
    ...
    if (hash != hashGenesisBlock) {
        ...
        if (nVersion >= 2)
        {
               ...
               (fTestNet && nHeight >= 400000))
            {
                ...
                if (vtx[0].vin[0].scriptSig.size() < expect.size() || vtx[1].vin[0].scriptSig.size() < expect.size() ||
                    !std::equal(expect.begin(), expect.end(), vtx[0].vin[0].scriptSig.begin()) ||
                    !std::equal(expect.begin(), expect.end(), vtx[1].vin[0].scriptSig.begin()))
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5ef756bea8ef0fe2bede9461d8c195c622f783b132b06ea3941c9e956639a6f9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "[26/01/2018] Soros o Bitcoinu: para na paru za diktatore, no blockchain tehnologija dobra za migrante";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 49 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("235684710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CTransaction txDonationsNew;
        txDonationsNew.vin.resize(1);
        txDonationsNew.vout.resize(1);
        txDonationsNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txDonationsNew.vout[0].nValue = 1 * COIN;
        txDonationsNew.vout[0].scriptPubKey = CScript() << ParseHex("235684710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.vtx.push_back(txDonationsNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517007600;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1546977; //4268434
        ...
        assert(block.hashMerkleRoot == uint256("0x63d999275c17039dc3bed2ddff9f0c874dd4ed21ad24b0d0ab89efcec4831058"));

        uint32_t n = vCrypt_N(0);
        block.print(n);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // vDinar: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, const CScript& scriptDonationsPubKeyIn)
{
    ...
    // Create donations coinbase tx
    CTransaction txDonationsNew;
    txDonationsNew.vin.resize(1);
    txDonationsNew.vin[0].prevout.SetNull();
    txDonationsNew.vout.resize(1);
    txDonationsNew.vout[0].scriptPubKey = scriptDonationsPubKeyIn;

    // Add our coinbase tx as first transaction
    pblock->vtx.push_back(txNew);
    // Add donations coinbase tx as second transaction
    pblock->vtx.push_back(txDonationsNew);
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = GetBlockReward(pindexPrev->nHeight+1, nFees);
        pblock->vtx[1].vout[0].nValue = GetBlockDonations(pindexPrev->nHeight+1, nFees);
        ...
        pblocktemplate->vTxSigOps[1] = pblock->vtx[1].GetLegacySigOpCount();
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey)
{
    ...
    //CPubKey donationspubkey;
    //donationspubkey.Set(donationskey.begin(), donationskey.end());
    //CBitcoinAddress scriptDonationsAddress;
    //scriptDonationsAddress.Set("318ac1c53c6ac049307f67e5e7b4649d855b2acb2bc0b63565b4e1e22a2ada1de");
    CBitcoinAddress donationsAddress;
    donationsAddress.SetString("DG1KpSsSXd3uitgwHaA1i6T1Bj1hWEwAxB");
    CScript scriptDonationsPubKey; // = CScript() << donationspubkey << OP_CHECKSIG;
    scriptDonationsPubKey.SetDestination(donationsAddress.Get());
    return CreateNewBlock(scriptPubKey, scriptDonationsPubKey);
}
...
void IncrementExtraNonce(CBlock* pblock, CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    pblock->vtx[1].vin[0].scriptSig = (CScript() << nHeight << CBigNum(nExtraNonce)) + COINBASE_FLAGS;
    assert(pblock->vtx[0].vin[0].scriptSig.size() <= 100);
    assert(pblock->vtx[1].vin[0].scriptSig.size() <= 100);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey, uint32_t nFactor)
{
 uint256 hash = pblock->GetPoWHash(nFactor);
 ...
 pblock->print(nFactor);
 printf("generated:\n");
 printf("%s for miner\n", FormatMoney(pblock->vtx[0].vout[0].nValue).c_str());
 printf("%s for donations\n", FormatMoney(pblock->vtx[1].vout[0].nValue).c_str());
 ...
 // Found a solution
 {
  ...
  {
   return error("vDinarMiner : generated block is stale");
  }
  ...
  {
   return error("vDinarMiner : ProcessBlock, block not accepted");
  }
 }
...
void static vDinarMiner(CWallet *pwallet)
{
    printf("vDinarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vdinar-miner");
    ...
    try { loop {
        ...
        printf("Running vDinarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        uint256 hashTarget = CBigNum().SetCompact(pblock->nBits).getuint256();
	uint32_t n = vCrypt_N(pindexPrev->nHeight + 1);
        loop
        {
            ...
            loop
            {
                ...
                if (thash <= hashTarget)
                {
                    ...
                    CheckWork(pblock, *pwallet, reservekey, n);
                    SetThreadPriority(THREAD_PRIORITY_LOWEST);
                    break;
                }
                pblock->nNonce += 1;
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&vDinarMiner, pwallet));
}
