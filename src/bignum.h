...
public:
    ...
    //DECLARED BOOLS
    CBigNum& SetCompactDeclared(unsigned int nCompact, bool *pfNegative = NULL, bool *pfOverflow = NULL)
    {
     ...
     {
      nWord >>= 8 * (3 - nSize);
      CBigNum b = CBigNum(nWord);
      if (!BN_copy(bn, b.bn))
      {
       BN_clear_free(bn);
       throw bignum_error("CBigNum::CBigNum& SetCompactDeclared() : BN_copy failed");
      }
     }
     else
     {
    ...
    void SetHex(const std::string& str)
    {
        ...
        {
         BN_clear_free(bn);
         throw bignum_error("CBigNum SetHex() : BN_copy failed");
        }
        while (isxdigit(*psz))
        {
            *this <<= 4;
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        std::string str;
        CBigNum b = *this;
        BN_set_negative(b.bn, false);
        CBigNum dv;
        CBigNum rem;
        if (BN_cmp(b.bn, b0.bn) == 0)
            return "0";
        while (BN_cmp(b.bn, b0.bn) > 0)
        {
            if (!BN_div(dv.bn, rem.bn, b.bn, bnBase.bn, pctx))
