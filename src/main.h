...
static const int64 DUST_SOFT_LIMIT = 100000; // 0.001 VDN
/** Dust Hard Limit, ignored as wallet inputs (mininput default) */
static const int64 DUST_HARD_LIMIT = 1000;   // 0.00001 VDN mininput
...
public:
    ...
    uint256 GetPoWHash(uint32_t nFactor) const
    {
        uint256 thash;
        vcrypt_1_1_256(BEGIN(nVersion), BEGIN(thash), nFactor);
        return thash;
    }
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos, uint32_t n)
    {
        SetNull();
        ...
        // Check the header
        if (!CheckProofOfWork(GetPoWHash(n), nBits))
    ...
    void print(uint32_t nFactor) const
    {
        printf("CBlock(hash=%s, input=%s, PoW=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, nFactor=%05u, vtx=%" PRIszu ")\n",
            GetHash().ToString().c_str(),
            HexStr(BEGIN(nVersion),BEGIN(nVersion)+80,false).c_str(),
            GetPoWHash(nFactor).ToString().c_str(),
            nVersion,
    ...
    // Context-independent validity checks
    bool CheckBlock(const CBlock* block, uint32_t nHeight, CValidationState &state, bool flexible, bool fCheckPOW=true, bool fCheckMerkleRoot=true) const;
