...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("vDinar version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  vdinard [options]                     " + "\n" +
                  "  vdinard [options] <command> [params]  " + _("Send command to -server or vdinard") + "\n" +
                  "  vdinard [options] help                " + _("List commands") + "\n" +
                  "  vdinard [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "vdinar:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: vdinar.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: vdinard.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the vDinar Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. vDinar is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("vDinar version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "vDinar server starting\n");
    ...
    if (mapArgs.count("-printblock"))
    {
        ...
        for (map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.begin(); mi != mapBlockIndex.end(); ++mi)
        {
            ...
            if (strncmp(hash.ToString().c_str(), strMatch.c_str(), strMatch.size()) == 0)
            {
                ...
                //COPYPASTE OF vCrypt_N(uint32_t n) FROM MAIN
                //Edit together with vCrypt_N!
                uint32_t n = 1024;
                for(uint32_t i = 10; (1 << i) < nBestHeight; i++)
                {
                    n = 1 << i;
                }

                //n FROM PREVIOUS vCrypt_N COPYPASTE
                block.print(n);
                printf("\n");
                nFound++;
            }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of vDinar") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart vDinar to complete") << "\n";
